...
// 这段组件实现了 Gift 管理页面，使用了 Ant Design 的 Tabs/Card/Form/Table 等
// - 三个 Tab：礼包接入、消息推送、活动接入
// - 按照 gift.md 的 element 还原主要模块、文案与交互
export default function GiftManagment(): React.ReactElement {
  ...
  // 删除 header 行
  const handleDeleteHeader = (rowKey: string) => {
  ...
  const headerColumns: ColumnsType<HeaderRow> = useMemo(() => ([
    {
      ...
      render: (_: string, record: HeaderRow) => (
        <Input
          ...
          onChange={(e) => updateHeaderCell(record.key, 'headerKey', e.target.value)}
        />
      )
    },
    {
      ...
      render: (_: string, record: HeaderRow) => (
        <Input
          ...
          onChange={(e) => updateHeaderCell(record.key, 'headerValue', e.target.value)}
        />
      )
    },
    {
      ...
      render: (_: unknown, record: HeaderRow) => (
        <Space>
          <Button type="link" icon={<DeleteOutlined />} onClick={() => handleDeleteHeader(record.key)}>
            删除
          </Button>
        </Space>
      )
    ...
    <div style={{ padding: 24 }}>
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, marginBottom: 16 }}>
        <div style={{ padding: '16px 24px' }}>
          ...
          <Space size={8} wrap>
            <Text type="secondary">
              礼包推送功能用于配置游戏的礼包发放和消息通知。支持通过API接入礼包资源，多语种推送消息，帮助游戏运营团队高效触达玩家。
            </Text>
            <Link href="https://w.g123.jp/doc/v2-bEVHpHXGZL" target="_blank" rel="noopener noreferrer">
              <u>查看操作文档</u>
            </Link>
          </Space>
        </div>
